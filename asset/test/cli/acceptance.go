...
func (am *Alertmanager) Terminate() {
	...
	if am.cmd.Process != nil {
		// Send SIGTERM to the process using os.Process.Signal
		if err := am.cmd.Process.Signal(syscall.SIGTERM); err != nil {
			am.t.Fatalf("Error sending SIGTERM to Alertmanager process: %v", err)
		}
	}
}
...
func (am *Alertmanager) Reload() {
	am.t.Helper()
	if am.cmd.Process != nil {
		// Send SIGHUP to the process using os.Process.Signal
		if err := am.cmd.Process.Signal(syscall.SIGHUP); err != nil {
			am.t.Fatalf("Error sending SIGHUP to Alertmanager process: %v", err)
		}
	}
}
